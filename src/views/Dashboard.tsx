...
const Dashboard: React.FC<DashboardProps> = ({ setView }) => {
  ...
  const roleNameMap: Record<ChatRole, string> = {
    ...
    Storyteller: t('dial.storyteller'),
    Playmate: t('dial.playmate'),
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8 flex-grow overflow-y-auto pb-8">
        ...
        <GlassWidget className="lg:col-span-2">
            ...
            <p className="text-secondary-text">{t('widgets.systemStatus.nominal')}</p>
             <h4 className="mt-4 font-bold text-lg text-primary-accent">Radiation Forecaster (3.2.2)</h4>
            <p className="text-sm text-yellow-400">Status: Elevated Solar Flux. Crew advised to limit EVA until 04:00 UTC.</p>
        </GlassWidget>
